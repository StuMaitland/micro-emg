...
void MainWindow::init()
{
    ...
    notchFilterFrequency = 50.0;//60.0;
    notchFilterBandwidth = 10.0;
    notchFilterEnabled = true;//false;
    ...
    setSaveFormat(SaveFormatFilePerChannel);// Modified by Bashar Awwad Shiekh Hasan 23/02/2015
    ...
    dacLockToSelectedBox->setChecked(true);
    dacSetChannel();
}
...
void MainWindow::createLayout()
{
    ...
    runExperimentButton = new QPushButton(tr("Run Expirement"));
    ...
    notchFilterComboBox->setCurrentIndex(1);
    ...
    //added to run the experiment
    connect(runExperimentButton, SIGNAL(clicked()), this, SLOT(runExperiment()));
    ...
    recordLayout->addWidget(runExperimentButton);
...
void MainWindow::openInterfaceBoard()
{
    ...
    string bitfilename =
     ...
     //string bitfilename = "D:\\Intan\\20140818-EVB100x-Release\\Bitfiles\\evb1005-xem6010-lx45.bit";
   // QMessageBox::information(this, tr("FPGA Configuration File Upload"),
     //                     tr("Trying to upload configuration file to FPGA."));
...
void MainWindow::runInterfaceBoard()
{
    ...
    while (running) {
        ...
        if (newDataReady) {
            ...
            for (int idx=0;idx<dataListeners.size();idx++){
                dataListeners.at(idx)->updateData(numUsbBlocksToRead);
            }
...
{
         QMessageBox msgBox9;
          msgBox9.setText(QString::fromAscii(e.what()));
          msgBox9.exec();
...
void MainWindow::saveImpedances()
{
    ...
    if (!csvFileName.isEmpty()) {
        saveImpedances(csvFileName);
    }
    wavePlot->setFocus();
}

void MainWindow::saveImpedances(QString csvFileName)
{
...
void MainWindow::runExperiment()
{
    ...
    ExperimentDialog experimentDialog(this,signalProcessor,lastMVC);
    ...
    if (experimentDialog.isCancelled()){
        wavePlot->setFocus();
        return;
    }

    saveBaseFileName = experimentDialog.getExperimentRHDFile();

    QMessageBox msgBox;
    msgBox.setText("Run Impedance!");
    ...
    if(experimentDialog.getImpedenceState()){
        if(!synthMode){
            //run the impdance measurment
            runImpedanceMeasurement();
            QString impFile= experimentDialog.getExperimentFolder()+"/_Impedances.csv";
            this->saveImpedances(impFile);
            ImpedanceView impView(this,impFile);
            impView.exec();

        }else{
            ImpedanceView impView(this,ImpedanceView::_DEMO);
            impView.exec();
        }
    }
...
void MainWindow::startRecordExperiment(){
...
void MainWindow::endRecording(){
    ...
    running=false;
    QString fname=ExperimentDetails::getDataFolder()+"/ExperimentDetails.txt";
    ExperimentDetails::saveExperiment(fname);
    runningExperiment=false;
    wavePlot->setFocus();
}

QVector<DataStreamListener*> MainWindow::dataListeners;
int MainWindow::dataListenersIds=0;

void MainWindow::addDataStreamListener(DataStreamListener * listener)
{
    //add a DataStreamListener so it can be called when new data block arrives
    listener->setId(++dataListenersIds);
    dataListeners.append(listener);
}

void MainWindow::removeDataStreamListener(DataStreamListener * listener)
{
    int idx=-1;
    for(int i=0;i<dataListeners.size();i++)
    {
        if(dataListeners.at(i)->getId()==listener->getId()){
            idx =i;
            break;
        }
    }
    if (idx==-1)
    {
        cout<<"Listener does not exists!"<<endl;
    }else{
        dataListeners.remove(idx);
        //cout<<"listener id:"<<listener->getId()<<" sucessfully removed";
    }
